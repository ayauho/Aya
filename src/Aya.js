...
//-- Expressions And Actions	
		...
		function func_call_start(false_if_started=false){
			...
			let defined = _.entityDefined(scope,entity_obj), r = false
			if(defined && defined.__data__.type=='function') r = true
		...
		function finish_recent_exprs(tab=false,value=null,postpone=false){				
			...
			if(postpone){
				...
				for(let i=_.level.expr-1;i>=0;i--){
					...
					else if(tab==='instruction_index' && expr[4]<_.instruction_index) break
		...
		function finish_recent_actions(tab=false,value=null,no_scope_start=false){
			...
			while(true){
				...
				else if(tab=='instruction_index' && ra[4]<_.instruction_index) break
...
//-- Check Entity
		...
		function free_to_assign(){
			return !_.entityDefined(scope,entity_obj) && assignable()
		}
		...
		function value(entity_){
			...
			let type = _.type(entity,remove_dots)
...
//-- General Management
		...
		function set_type_to_assignments(type,level){			
			let n = _.entity_index-1,w=_.writes[n]
			while(true){
				w=_.writes[n]
				if(!w.start.var_assignment) break
				if(w.level.assignment<=level&&w.assignment.stage<3){
					_.writes[n].scope[w.entity].__data__.type=type
				}
				n--
				if(!_.writes[n]) break
			}
...
//-- Parse Lines			
			...
			for(d=0; d<rangeStruct.length; d++){
				...
				for(index=0; index<line[1].length; ){
					...
					if((prev_entity_obj.class_name||prev_entity_obj.extends)&&lineFirstEntity){
						...
						_.defineEntity(scope,'this','object',true)
					...
					if(!['...','..','.'].includes(entity)&&middle_dots_count&&middle_dots_count!=2&&!literal()&&(!dots_count_before||prev_entity_obj.start.key_expression)){
						try{
							...
							if(entity_obj.start.var_assignment) entity_obj.chain_assignment = chain_assignment = true
					...
					if(bracket(entity)){
						if(left_bracket(entity)&&!operator(-1)){
							start()
							if(prev_entity_obj.end.func_decl||prev_entity==double_comma&&_.writes[_.entity_index-2].end.func_decl){
					...
					if(!skip1 
						...
						if(anon_func.includes(entity) || 
							...
							&& (assignable(entity)) 
							&& !anonimous_function(next_entity) && !colon_count && (lineLastEntity || first_symbol(next_entity)==backslash || set_get.includes(prev_entity))){							
							...
							if(!entity_obj.anonimous){
								helper = recentAction=='class'&&(!prev_entity_obj.static||prev_entity_obj.is_get&&!_.writes[_.entity_index-2].static)? true:false
								if(helper) {
									entity_obj.scope = scope['this']
								} else entity_obj.scope = scope				
								_.defineEntity(entity_obj.scope, entity_obj, entity_obj.arrow?'arrow_function':'function', helper)							
						...
						if( !anon_func.includes(entity) && colon_count==2 && !entity || recent_action()[0]!='immediate_func_call'&&recent_action()[0]!='object_assignment' && (!chain||chain_assignment) && (assignable()||chain_assignment) && !func_call_start() && !func_decl_start() && value(next_entity) && (!lineLastEntity || dots_count==2) && !no_define || colon_count==2){
							if(dots_count==2) {								
								...
								if(helper = entity_obj.defined) {
									...
									else helper.__data__.type='array'
								}
							}
							...
							if(!chain_assignment&&free_to_assign()&&next_entity!=sub){								
								helper = recentAction=='class'&&(!prev_entity_obj.static||prev_entity_obj.is_get&&!_.writes[_.entity_index-2].static)? true:false
								if(helper) entity_obj.scope = scope['this']
					...
					if(assignable() || literal() || chained()){												
						if(!func_call_start()){
							try{
								...
								if(!entity_obj.next_entity_operator){
									end(true)
								}
							} catch(err) {
								_.errorType(1,_.line_index)
											...
											if(_scope.this){
												...
												for(let i2=0;i2<2;i2++){
													...
													for(let i in where[i2]) {
														if(i!='__data__'&&!where[i2][i].__data__.maybe)
															_scope[i]=where[i2][i]
													}
													if(i2) {
														delete _scope.this
										...
										if(is_emit){
											...
											if(_scope&&!_.writes[n].assigned){
												if(_.writes[n].chain&&_.writes[n].defined) {
													...
													_.writes[n].defined.__data__.type='object'
	...
	async prepareJS(){
		let _=this,show_logs=true,debugJs={assignment:[],func_call:[],if:{},else_if:{},else:{}}, closed_actions={}
		...
		function add_bottom_comments(w){
			if(w.added_bottom_comments || !_.aya_glob_data.struct[w.struct_index][2][2][0]) return ''
...
//-- entity/converted
				...
				if(check&&!w.hidden || !check&&w.hidden){
					...
					for(let d of actions_started){
						...
						if(!(w.hide_else_close&&d[1]=='else')) {											
							for(let name of d[1]) {
								if(name=='array_assignment'&&p.push) {
									continue
								}
				...
				if(w.has_semicolon&&(!n||n&&!n.preceding_comma)&&!w.end.immediate_call&&!w.end.immediate_func_call){
					js+=";\n"
					addBottomComments=true
				}
	...
	linesToStruct(lines){
		...
		let sign,sign2,variants=[],variants2=[],pairs=[],a,b,bf='',af='',pat, str_assoc={},multi_comment,multi_comment_started,side_comment='',top_comments=[], comments_arr=[]

		for(l=0;l<lines.length;l++){
			line = lines[l]
			if(side_comment){
				top_comments.push(side_comment)
				side_comment=''
			}			
			if(multi_comment){
				m = /\*\//.exec(line)
				if(!m){
					...
					side_comment=line.substring(0,m.index+2).trim()
					line=line.replace(side_comment,'')
					multi_comment=false
				}
			}
			while(true){
			...
			while(true){
				...
				if(m){
					...
					if(m2){
						side_comment = line.substring(m.index,m2.index+2)				
					} else {
						side_comment=line.substring(m.index).trim()
						top_comments.push(side_comment)
						multi_comment=true									
					}
					line=line.replace(side_comment,'')					
			...
			for(let e in es){								
				m = es[e].match(/\$\\\\(\d)/)
				if(m) es[e] = es[e].replace('$\\\\'+m[1],ss2[m[1]])
				if(es[e].includes('$\\')) {
			...
			comments_arr = [side_comment,[],[]]
			if(!multi_comment_started) {
				if(struct.length&&prev_d==d) struct[struct.length-1][2][2]=top_comments
				else comments_arr[1] = top_comments
				top_comments=[]
	...
	literal(entity){
		...
		else if(entity[0]=='/'&&entity[entity.length-1]=='/') type='regex'
		else type = false
		return type
	}
	...
	entityDefined(scope,entity,not_object_scope=false,include_assignment=false){
		...
		if(typeof entity=='object'){
			...
			if(entity.defined&&(!not_object_scope||!scope.__data__.objectScope)) return entity.defined
			entity_=entity.entity
		}
		let _=this,r=null, untill, untill_function, untill_window
		...
		if(entity_=='this') {
			while(true){
				if(!['function','class'].includes(_scope.__data__.type) || _scope.__data__.type=='object'&&not_object_scope || _scope.__data__.classEntity || _scope.__data__.arrow) {
		...
		while(true){			
			...
			if(_scope[entity_]&&!_scope[entity_].__data__.inProcess&&(!not_object_scope || !_scope.__data__.objectScope) || _scope.is_window_scope && typeof window[entity_]!='undefined' ) {
				if(!include_assignment || !(assignments.scope&&_scope.__data__.id == assignments.scope.__data__.id&&includes))
					if(_scope[entity_]&&!_scope[entity_].__data__.maybe || _scope.is_window_scope) {
						r = _scope.is_window_scope&&(!_scope[entity_])?{__data__:{parentScope:_scope,type:typeof window[entity_],inWindow: window[entity_]}}:_scope[entity_]
						break
					}
			}
			if(_scope.__data__.inWindow&&_scope.__data__.inWindow[entity_]){
				r = {__data__:{parentScope:_scope,type:typeof _scope.__data__.inWindow[entity_],inWindow: _scope.__data__.inWindow[entity_]}}
				break
			}
		...
		if(untill_window && r && _scope.is_window_scope) {
			r=null
		}	
		return r
	}

	defineEntity(scope,entity,type=null,dont_assign=false,as_argument=false,inProcess=false){
		...
		if(entity_!='this' && !scope[entity_] && !dont_assign && type!='function' && !scope.__data__.objectScope || type=='arrow_function'){
			if(!as_argument) {
	...
	inAssignmentProcess(entity_scope,entity){
		return entity_scope&&entity_scope.__data__.inProcess || this.record().entities&&this.record().entities.includes(entity)
	}	
	...
	chaining(entity_obj,scope,as_value=false,contine=false){
		...
		function work_chain(parent,descendant,index,converted){
			...
			if(parent[0]!='`') {
				parent_defined = _.entityDefined(index?scope.__data__.parentScope:scope,parent,true)
				if(!index&&parent_defined) scope = parent_defined.__data__.parentScope
				if(parent_defined) {
			...
			if(!literal_type) {
				if(parent_defined&&eval( `window.${parent} && typeof window.${parent}.${descendant}=='function'`) || typeof [parent][descendant]=='function' || ['function','object'].includes(parent_type) && typeof (function(){})[descendant]=='function'){					
					...
					descendant_defined = _.entityDefined(scope,descendant,'window')
					...
					if(!descendant_defined) {
						...
						else if(descendant_type!='function') _.error(5,descendant_type,descendant)
					}
				}
			} else {
				if((!['object','function'].includes(parent_type) || !parent_defined) && parent!='this' && !prev_descendant_defined && !as_argument) {
			...
			if(!parent_defined || define_rest || (parent=='this'&&!parent_scope['this']) || force_assign){
				if(!parent_defined || parent_defined.__data__.type!=parent_type || parent_defined.__data__.maybe || define_rest || parent=='this'){
					if(!parent_defined || parent=='this') {
						parent_defined_recently = parent_defined = _.defineEntity(scope,parent,parent_type,index,as_argument)
						scope = parent_defined
				...
				if(index && chain_def=='var_assignment' && (define_rest && parent!='this' && !prev_descendant_defined && !imported_object || force_assign && (!parent_defined || parent_defined_recently))){					
			...
			if(['object','function'].includes(descendant_type)||right_var_name&&parent_type!='array') descendant = `.${descendant}`
			else descendant = `[${descendant}]`
		...
		function continue_chain(entity){
			...
			entity_defined = _.entityDefined(scope,entity)
			if(entity_defined) {
		...
		if(scope && scope.__data__.type=='function'&& chain_def!='var_assignment'&&!entity_obj.func_decl) {
			chain_def='func_call'
		}
		if(chain_def) entity_obj.start[chain_def] = true
		if(parts[0]!='this'&&entity_obj.initial_assignment&&chain_def=='func_call') _.error(10,entity)
		if(chain_def!='func_call')entity_obj.defined = scope
	...
	type(entity,remove_dots=false){
		...
		let concrete, r
		if(concrete=_.literal(entity)) r={type:'literal',concrete}
